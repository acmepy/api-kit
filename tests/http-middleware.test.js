...
describe("http middleware", () => {
  ...
  it("serves the client example using only the client package for API calls", async () => {
    ...
    try {
      ...
      assert.match(asset.raw, /services\.ventas\.list\(\)/);
      assert.match(asset.raw, /services\.clientes\.pull\(\)/);
      assert.match(asset.raw, /services\.ventas\.pull\(\)/);
      ...
      assert.match(asset.raw, /client\.service\("pending"\)/);
      ...
      assert.match(asset.raw, /service\.push/);
      assert.match(asset.raw, /services\.clientes\.push/);
      assert.match(asset.raw, /savePending/);
      assert.match(asset.raw, /services\.pending\.update/);
      assert.match(asset.raw, /deletePending/);
      assert.match(asset.raw, /loadCachedLists/);
      assert.doesNotMatch(asset.raw, /client\.syncServices/);
      assert.doesNotMatch(asset.raw, /client\.pending/);
      assert.doesNotMatch(asset.raw, new RegExp("client\\." + "service" + "Data"));
